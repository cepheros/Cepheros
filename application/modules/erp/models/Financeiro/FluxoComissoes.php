...
class Erp_Model_Financeiro_FluxoComissoes extends Zend_Db_Table_Abstract{

	protected $_name = 'tblcomissoes_fluxo';
	protected $_primary = 'id_registro';
	
	
	public static function saldoAtual($vendedor, $format = 2){
		$dados = new Erp_Model_Financeiro_FluxoComissoes();
		$return = $dados->getAdapter()->select()
		->from(array('a'=>'tblcomissoes_fluxo'),array('SUM(a.valorregistro) as valor'))
	...
	public static function saldoAtualComEste($lanc, $vendedor, $format = 2){
		$lanc = $lanc+1;
		$dados = new Erp_Model_Financeiro_FluxoComissoes();
		$return = $dados->getAdapter()->select()
		->from(array('a'=>'tblcomissoes_fluxo'),array('SUM(a.valorregistro) as valor'))
		->where("id_vendedor = '$vendedor' and id_registro < '$lanc' ");
