...
class Erp_Model_Faturamento_Perfil extends Zend_Db_Table_Abstract{
	
	protected $_name = 'tblperfilfaturamento_basicos';
	...
	public static function getCombo(){
		...
		foreach($dados as $depto){
			$rdepto[$depto['id_registro']] = $depto['nomedoperfil'];
		}
		return $rdepto;
	}
	...
	public static function getNaturezaOperacao($id){
		$db1 = new Erp_Model_Faturamento_Perfil();
		$dados = $db1->fetchRow("id_registro = '$id'");
		return $dados->naturezaoperacao;		
		
	}
	
	public static function getObsFisco($id){
		$db1 = new Erp_Model_Faturamento_Perfil();
		$dados = $db1->fetchRow("id_registro = '$id'");
		return $dados->observacoesfisco;
	
	}
	
	public static function  getDataOperacao($id){
